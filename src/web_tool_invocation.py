...
from src.data_extraction_other_cai import load_whole_data_from_file as load_whole_data_from_file_cai
...
def topic_prompt_volume_and_web_rate_over_time(df, top_k=5):
    ...
    df["has_web_call"] = df["interactions"].apply(_has_web_call_interaction)
    df["month"] = pd.to_datetime(df["month"], errors="coerce")
    df = df[df["month"].notna()].copy()
...
def _has_web_call_interaction(interactions):
    return any("web" in str(item).lower() for item in _as_list_value(interactions))
...
def _has_web_call_for_platform(row, platform):
    if platform in {"openai", "chatgpt"}:
        return _has_web_call_interaction(row.get("interactions"))

    tools = _as_list_value(row.get("tools"))
    if platform == "claude":
        return any("web" in str(tool).lower() for tool in tools)
    if platform == "grok":
        return any(str(tool) in GROK_WEB_TOOLS for tool in tools)
    if platform == "deepseek":
        return any(str(tool) in DEEPSEEK_WEB_TOOLS for tool in tools)
    return False
...
def topic_distriction_of_whole_data_all_platforms(platform_configs=None):
    ...
    for platform, display_name in platform_configs:
        ...
        if platform in {"openai", "chatgpt"} and "interactions" not in df.columns:
            print(f"Skipping `{platform}` because `interactions` column is missing.")
            continue
        if platform not in {"openai", "chatgpt"} and "tools" not in df.columns:
...
def policy_distribution_over_time(
    input_path=None,
    file_name="web_call_policy_characterization_over_time",
):
    input_path = input_path or f"{OUTPUT_PATH}/metadata/web_calls_characterization.csv"
    df = pd.read_csv(input_path)
    if "followed_web_policy" not in df.columns or "time" not in df.columns:
        print("Missing `followed_web_policy` or `time` column.")
        return pd.DataFrame()
        ...
        plot_rows.append(
            {
                ...
                "policy": primary_trigger,
                "policy_display": policy_label_map.get(
                    primary_trigger,
                    primary_trigger,
                ),
            }
    ...
    if plot_df.empty:
        print("No valid policy rows found for temporal plot.")
        return pd.DataFrame()

    policy_order = (
        plot_df["policy_display"]
        .value_counts()
        .rename_axis("policy_display")
        .reset_index(name="total_count")["policy_display"]
        .tolist()
    )
    monthly_counts = (
        plot_df.groupby(["month", "policy_display"])
        .size()
        .reset_index(name="count")
    )
    all_months = sorted(plot_df["month"].unique())
    full_index = pd.MultiIndex.from_product(
        [all_months, policy_order],
        names=["month", "policy_display"],
    )
    monthly_counts = (
        monthly_counts.set_index(["month", "policy_display"])
        .reindex(full_index, fill_value=0)
        .reset_index()
    )
    monthly_counts["total"] = monthly_counts.groupby("month")["count"].transform("sum")
    monthly_counts["percentage"] = monthly_counts.apply(
    ...
    monthly_counts.to_csv(
        f"{OUTPUT_PATH}/metadata/{file_name}.csv",
        index=False,
    )

    fig = go.Figure()
    color_sequence = qualitative.Plotly
    for idx, policy_display in enumerate(policy_order):
        policy_df = monthly_counts[
            monthly_counts["policy_display"] == policy_display
        ].sort_values("month")
        fig.add_trace(
            go.Scatter(
                x=policy_df["month"],
                y=policy_df["percentage"],
                mode="lines+markers",
                name=policy_display,
                line=dict(width=3, color=color_sequence[idx % len(color_sequence)]),
                marker=dict(size=7),
                customdata=policy_df[["count", "total"]].values,
                hovertemplate=(
    ...
    fig.update_layout(
        ...
        yaxis_title="Percentage",
        legend_title="",
        margin=dict(l=70, r=30, t=50, b=90),
    )
    fig.update_xaxes(
        dtick="M2",
        tickformat="%b %Y",
    ...
    fig.write_html(f"{OUTPUT_PATH}/{CONF}/{file_name}.html")
    fig = with_paper_style(fig, config=styler(18, 16), legend_pos=(0.9, 1.45))
    fig.write_image(f"{OUTPUT_PATH}/{CONF}/{file_name}.pdf", format="pdf")
...
def _replace_cai_policy_topics_from_web_data_summary(df, platform):
    if platform in {"openai", "chatgpt"}:
        return df

    web_data_path = Path(OUTPUT_PATH) / platform / "metadata" / "web_data_summary.csv"
    if not web_data_path.exists():
        print(f"Missing web data summary file for `{platform}`: {web_data_path}")
        return df

    web_data_df = pd.read_csv(web_data_path)
    key_columns = [
        col
        for col in ["user_id", "conv_id", "turn_id"]
        if col in df.columns and col in web_data_df.columns
    ]
    if not key_columns:
        print(f"No shared key columns found for `{platform}`.")
        return df
    if "topic" not in df.columns or "topic" not in web_data_df.columns:
        print(f"`topic` column missing for `{platform}`.")
        return df

    topic_lookup_df = (
        web_data_df[key_columns + ["topic"]]
        .dropna(subset=key_columns)
        .drop_duplicates(subset=key_columns, keep="first")
        .rename(columns={"topic": "topic_from_web_data_summary"})
    )
    merged_df = df.merge(topic_lookup_df, on=key_columns, how="left")
    matched_mask = merged_df["topic_from_web_data_summary"].notna()
    merged_df.loc[matched_mask, "topic"] = merged_df.loc[
        matched_mask,
        "topic_from_web_data_summary",
    ]
    print(
        f"{platform}: replaced topics for {int(matched_mask.sum())}/"
        f"{len(merged_df)} policy rows from web_data_summary.csv."
    )
    return merged_df.drop(columns=["topic_from_web_data_summary"])


def _load_policy_characterization_df_for_platform(platform):
    if platform in {"openai", "chatgpt"}:
        candidate_paths = [
        ...
        candidate_paths = [
            f"{OUTPUT_PATH}/{platform}/metadata/web_calls_characterization.csv",
        ]
    ...
    raise FileNotFoundError(
        f"No web-call characterization file found for `{platform}`. "
        f"Tried: {candidate_paths}"
    )


def _cramers_v_from_crosstab(crosstab):
    if crosstab.empty:
        return 0.0

    observed = crosstab.astype(float)
    ...
    expected = pd.DataFrame(
        {
            col: row_totals * col_total / n
            for col, col_total in col_totals.items()
        },
        index=observed.index,
    )
    expected = expected.replace(0, pd.NA)
    chi2 = (((observed - expected) ** 2) / expected).fillna(0).to_numpy().sum()
...
def topic_policy_correlation_all_platforms(platform_configs=None):
    if platform_configs is None:
        platform_configs = [
            ("openai", "OpenAI"),
            ("claude", "Claude"),
    ...
    excluded_topics = {"other", "uncategorized", "physics"}
    excluded_policies = {"None of the Above", "OpenAI Product Info", ""}
    records = []
    summary_rows = []
    source_paths = {}
    ...
    for platform, display_name in platform_configs:
        try:
            df, source_path = _load_policy_characterization_df_for_platform(platform)
        except Exception as e:
            print(f"Failed to load policy characterization for `{platform}`: {e}")
            continue

        df = _replace_cai_policy_topics_from_web_data_summary(df, platform)
        source_paths[platform] = source_path
        if "topic" not in df.columns or "followed_web_policy" not in df.columns:
            print(
                f"Skipping `{platform}` because `topic` or "
                "`followed_web_policy` is missing."
            )
            continue
        ...
        for _, row in df.iterrows():
            ...
            if not topic or topic.lower() in excluded_topics:
                continue

            expected_policy = topic_to_situation_mapping.get(topic)
            if not expected_policy:
                unmapped_topics[topic] += 1
                continue

            policy = _parse_followed_web_policy(row.get("followed_web_policy"))
            observed_policy = str(policy.get("primary_trigger", "") or "").strip()
            if observed_policy in excluded_policies:
                continue
            ...
            record = {
                ...
                "topic": topic,
                "mapped_policy": expected_policy,
                "primary_situation": observed_policy,
                "matches_mapping": expected_policy == observed_policy,
            }
            ...
            summary_rows.append(
                {
                    ...
                    "num_rows": 0,
                    "match_rate": 0.0,
                    "mapped_policy_primary_situation_cramers_v": 0.0,
                    "num_unmapped_topics": int(sum(unmapped_topics.values())),
                }
            )
            continue

        mapped_policy_table = pd.crosstab(
            platform_df["mapped_policy"],
            platform_df["primary_situation"],
        )
        summary_rows.append(
            {
                "platform": platform,
                "platform_display": display_name,
                "num_rows": int(len(platform_df)),
                "match_rate": float(platform_df["matches_mapping"].mean()),
                "mapped_policy_primary_situation_cramers_v": _cramers_v_from_crosstab(
                    mapped_policy_table
                ),
                "num_unmapped_topics": int(sum(unmapped_topics.values())),
            }
    ...
    if not records:
        print("No topic-policy records found.")
        return pd.DataFrame()
    ...
    file_name = "topic_policy_correlation_all_platforms"
    records_df.to_csv(f"{OUTPUT_PATH}/metadata/{file_name}_records.csv", index=False)
    summary_df.to_csv(f"{OUTPUT_PATH}/metadata/{file_name}_summary.csv", index=False)
    for platform, platform_records in records_df.groupby("platform"):
        crosstab = pd.crosstab(
            platform_records["mapped_policy"],
            platform_records["primary_situation"],
        )
        crosstab.to_csv(
            f"{OUTPUT_PATH}/metadata/{file_name}_{platform}_mapped_policy_primary_situation_crosstab.csv"
        )
    to_json(
        source_paths,
        f"{OUTPUT_PATH}/metadata/{file_name}_sources.json",
    )

    match_pct_by_topic = (
        records_df.groupby(["topic", "platform_display"])["matches_mapping"]
        .mean()
        .mul(100)
        .reset_index(name="match_percentage")
        .pivot(index="topic", columns="platform_display", values="match_percentage")
    )
    openai_sort = (
        match_pct_by_topic["OpenAI"].sort_values(ascending=False).index.tolist()
        if "OpenAI" in match_pct_by_topic.columns
        else match_pct_by_topic.mean(axis=1).sort_values(ascending=False).index.tolist()
    )
    match_pct_by_topic = match_pct_by_topic.reindex(openai_sort)
    match_pct_by_topic.to_csv(
        f"{OUTPUT_PATH}/metadata/{file_name}_match_percentages_by_topic.csv"
    )

    print("\nMapped-policy vs primary-situation correlation summary:")
    print(summary_df.to_string(index=False))
    print("\nTopic mapping match percentage by platform:")
    print(match_pct_by_topic.round(2).fillna("").to_string())
    return summary_df
...
def replay_call_outcome_venn_diagram(
    ...
    result = binomtest(all_tools_count+no_tool_count, len(merged_df))
    ci = result.proportion_ci(confidence_level=0.95, method="wilson")

    print(
        f"Agreement: {100*(all_tools_count+ no_tool_count)/len(merged_df):.2f}% "
        f"(95% CI: {100*ci.low:.2f}%–{100*ci.high:.2f}%)"
    )
    ...
    y_labels = ["ChatGPT<br>Not Called<br>Web Search", "ChatGPT<br>Called<br>Web Search"]
...
def replay_model_call_outcome_venn_diagram(
    ...
    base_model_label="GPT-5.3",
    replay_dir=None,
):
    merge_keys = ["conv_id", "turn_id"]
    replay_dir = Path(replay_dir or f"{OUTPUT_PATH}/replays")
    ...
    def _replay_result_called_web(result):
    ...
    replay_model_names = list(dict.fromkeys(replay_model_names))
    if base_model_name not in replay_model_names:
        replay_model_names = [base_model_name] + replay_model_names

    def _load_model_df(model_name):
        file_path = replay_dir / f"{model_name}.json"
        if not file_path.exists():
            print(f"Missing replay file for {_model_label(model_name)}: {file_path}")
            return pd.DataFrame()

        replay_data = load_json(file_path)
        if not replay_data:
            return pd.DataFrame()
        ...
        for result in replay_rows:
            ...
            row = {
                ...
                model_name: _replay_result_called_web(result.get("auto", {})),
            }
            if all(str(row.get(col, "")).strip() != "" for col in merge_keys):
                records.append(row)

        df = pd.DataFrame(records)
        if len(df) == 0:
            print(f"No usable replay rows for {_model_label(model_name)}.")
            return pd.DataFrame()

        duplicate_rows = int(df.duplicated(subset=merge_keys).sum())
        if duplicate_rows:
            print(
                f"Dropped {duplicate_rows} duplicate replay rows for "
                f"{_model_label(model_name)}."
            )
            df = df.drop_duplicates(subset=merge_keys).copy()
        df = df.drop_duplicates(subset=merge_keys).copy()
        return df[merge_keys + [model_name]].copy()

    model_dfs = []
    for model_name in replay_model_names:
        model_df = _load_model_df(model_name)
        if len(model_df) == 0:
            continue
        model_dfs.append(model_df)

    if not model_dfs:
        print("No model evaluation files available.")
        return pd.DataFrame()

    merged_df = model_dfs[0]
    for model_df in model_dfs[1:]:
        merged_df = merged_df.merge(model_df, on=merge_keys, how="inner")

    if base_model_name not in merged_df.columns:
        print(f"Missing base model after merge: {base_model_name}")
        return pd.DataFrame()

    comparison_models = [
        model_name
        for model_name in replay_model_names
        if model_name != base_model_name and model_name in merged_df.columns
    ]
    if not comparison_models:
        print("No comparison models available after merge.")
        return pd.DataFrame()

    base_called = merged_df[base_model_name].astype(bool)
    base_not_called = ~base_called

    summary_rows = []
    for model_name in comparison_models:
        model_called = merged_df[model_name].astype(bool)
        model_not_called = ~model_called
        summary_rows.append(
            {
                "model": _model_label(model_name),
                "base_called_and_model_called": int((base_called & model_called).sum()),
                "base_called_and_model_not_called": int(
                    (base_called & model_not_called).sum()
                ),
                "base_not_called_and_model_not_called": int(
                    (base_not_called & model_not_called).sum()
                ),
                "base_not_called_and_model_called": int(
                    (base_not_called & model_called).sum()
                ),
            }
        )

    summary_df = pd.DataFrame(summary_rows)
    print(f"Common samples across replay models: {len(merged_df)}")
    print(summary_df.to_string(index=False))

    output_dir = f"{OUTPUT_PATH}/{CONF}/replay_modes/"
    os.makedirs(output_dir, exist_ok=True)
    safe_base = str(base_model_name).replace("/", "__")
    file_name = f"replay_models_auto_web_call_base_{safe_base}"
    summary_df.to_csv(f"{output_dir}/{file_name}_summary.csv", index=False)
    to_json(
        summary_df.to_dict(orient="records"),
        f"{output_dir}/{file_name}_summary.json",
    ...
    for model_name in comparison_models:
        ...
        plot_rows.extend(
            [
                {
                    "model": model_label,
                    "outcome": f"{base_model_label} called, model called",
                    "count": int((base_called & model_called).sum()),
                    "base_decision": "Called Web Search",
                },
                {
                    "model": model_label,
                    "outcome": f"{base_model_label} called, model did not",
                    "count": int((base_called & model_not_called).sum()),
                    "base_decision": "Called Web Search",
                },
                {
                    "model": model_label,
                    "outcome": f"{base_model_label} did not, model did not",
                    "count": int((base_not_called & model_not_called).sum()),
                    "base_decision": "Did Not Call Web Search",
                },
                {
                    "model": model_label,
                    "outcome": f"{base_model_label} did not, model called",
                    "count": int((base_not_called & model_called).sum()),
                    "base_decision": "Did Not Call Web Search",
                },
            ]
        )
    ...
    outcome_specs = [
        (f"{base_model_label} called, model called", "#2ca02c"),
        (f"{base_model_label} called, model did not", "#d62728"),
        (f"{base_model_label} did not, model did not", "#1f77b4"),
        (f"{base_model_label} did not, model called", "#ff7f0e"),
    ]

    fig = go.Figure()
    for outcome, color in outcome_specs:
        subset = plot_df[plot_df["outcome"] == outcome].set_index("model")
        values = [int(subset.loc[label, "count"]) for label in y_labels]
        fig.add_trace(
            go.Bar(
                y=y_labels,
                x=values,
...
def replay_system_prompt_call_outcome_venn_diagram(
    replay_model_name="gpt-5-mini-2025-08-07",
    ...
    baseline_label="No System Prompt",
    replay_dir=None,
    web_tool_name=None,
):
    merge_keys = ["conv_id", "turn_id"]
    ...
    prompt_label_map = {
        ...
        "web_policy_detailed": "Detailed Policy",
        "web_policy_compact": "Compact Policy",
        "web_policy_avoid": "Avoid Web Policy",
        "web_policy_never": "Never Web Policy",
    }
    ...
    def _prompt_label(prompt_version):
        return prompt_label_map.get(prompt_version, str(prompt_version))

    def _resolved_web_tool_name():
        tool_aliases = {
            "google": "mcp-serp",
            "serp": "mcp-serp",
            "serpapi": "mcp-serp",
        ...
        tool_name = (
            web_tool_name
            or os.getenv("REPLAY_WEB_TOOL_NAME", "").strip()
            or os.getenv("WEB_TOOL_NAME", "").strip()
            or os.getenv("MCP_SEARCH_PROVIDER", "").strip()
        )
        if not tool_name:
            return None
        return tool_aliases.get(str(tool_name).strip().lower(), tool_name)

    def _tool_suffix():
        tool_name = _resolved_web_tool_name()
        if not tool_name:
            return None
        return _safe_filename(tool_name)

    def _replay_file_candidates_for_prompt(prompt_version):
        tool_suffix = _tool_suffix()
        if prompt_version == "none":
            candidates = []
            if tool_suffix:
                candidates.append(
                    replay_dir / f"{replay_model_name}__tool-{tool_suffix}.json"
                )
            candidates.append(replay_dir / f"{replay_model_name}.json")
            return candidates
        suffix = _safe_filename(prompt_version)
        candidates = []
        if tool_suffix:
            candidates.append(
                replay_dir
                / f"{replay_model_name}__system-{suffix}__tool-{tool_suffix}.json"
            )
        candidates.append(replay_dir / f"{replay_model_name}__system-{suffix}.json")
        return candidates

    def _replay_file_for_prompt(prompt_version):
        candidates = _replay_file_candidates_for_prompt(prompt_version)
    ...
    def _replay_result_called_web(result):
        ...
        return any(
            ...
            and item.get("type") in {"web_search_call", "mcp_call"}
            for item in output_items
        )

    prompt_versions = list(dict.fromkeys(system_prompt_versions))
    if baseline_prompt_version in prompt_versions:
        prompt_versions = [
            version for version in prompt_versions if version != baseline_prompt_version
        ]
    all_prompt_versions = [baseline_prompt_version] + prompt_versions

    def _load_prompt_df(prompt_version):
        file_path = _replay_file_for_prompt(prompt_version)
        if not file_path.exists():
            print(f"Missing replay file for {_prompt_label(prompt_version)}: {file_path}")
            return pd.DataFrame()

        replay_data = load_json(file_path)
        if not replay_data:
            print(f"Empty replay file for {_prompt_label(prompt_version)}: {file_path}")
            return pd.DataFrame()
        ...
        for result in replay_rows:
            ...
            row = {
                "conv_id": result.get("conv_id", ""),
                "turn_id": result.get("turn_id", ""),
                prompt_version: _replay_result_called_web(result.get("auto", {})),
            }
            if all(str(row.get(col, "")).strip() != "" for col in merge_keys):
                records.append(row)
        ...
        df = pd.DataFrame(records)
        if len(df) == 0:
            print(f"No usable replay rows for {_prompt_label(prompt_version)}.")
            return pd.DataFrame()
        ...
        if duplicate_rows:
            print(
                f"Dropped {duplicate_rows} duplicate replay rows for "
                f"{_prompt_label(prompt_version)}."
            )
            df = df.drop_duplicates(subset=merge_keys).copy()
        return df[merge_keys + [prompt_version]].copy()

    prompt_dfs = []
    for prompt_version in all_prompt_versions:
        prompt_df = _load_prompt_df(prompt_version)
        if len(prompt_df) == 0:
            continue
        prompt_dfs.append(prompt_df)

    if not prompt_dfs:
        print("No system-prompt replay files available.")
        return pd.DataFrame()

    merged_df = prompt_dfs[0]
    for prompt_df in prompt_dfs[1:]:
        merged_df = merged_df.merge(prompt_df, on=merge_keys, how="inner")

    if baseline_prompt_version not in merged_df.columns:
        print(f"Missing baseline prompt after merge: {baseline_prompt_version}")
        return pd.DataFrame()

    comparison_versions = [
        prompt_version
        for prompt_version in prompt_versions
        if prompt_version in merged_df.columns
    ]
    if not comparison_versions:
        print("No comparison system prompts available after merge.")
        return pd.DataFrame()

    baseline_called = merged_df[baseline_prompt_version].astype(bool)
    baseline_not_called = ~baseline_called

    summary_rows = []
    for prompt_version in comparison_versions:
        prompt_called = merged_df[prompt_version].astype(bool)
        prompt_not_called = ~prompt_called
        summary_rows.append(
            {
                "system_prompt_version": prompt_version,
                "system_prompt_label": _prompt_label(prompt_version),
                "baseline_called_and_prompt_called": int(
                    (baseline_called & prompt_called).sum()
                ),
                "baseline_called_and_prompt_not_called": int(
                    (baseline_called & prompt_not_called).sum()
                ),
                "baseline_not_called_and_prompt_not_called": int(
                    (baseline_not_called & prompt_not_called).sum()
                ),
                "baseline_not_called_and_prompt_called": int(
                    (baseline_not_called & prompt_called).sum()
                ),
            }
        )

    summary_df = pd.DataFrame(summary_rows)
    print(f"Common samples across system-prompt versions: {len(merged_df)}")
    print(summary_df.to_string(index=False))

    output_dir = f"{OUTPUT_PATH}/{CONF}/replay_modes/"
    os.makedirs(output_dir, exist_ok=True)
    safe_model = str(replay_model_name).replace("/", "__")
    safe_baseline = _safe_filename(baseline_prompt_version)
    tool_suffix = _tool_suffix()
    tool_name_part = f"_tool_{tool_suffix}" if tool_suffix else ""
    file_name = (
        f"replay_system_prompts_auto_web_call_{safe_model}_"
        f"baseline_{safe_baseline}{tool_name_part}"
    )
    summary_df.to_csv(f"{output_dir}/{file_name}_summary.csv", index=False)
    to_json(
        summary_df.to_dict(orient="records"),
    ...
    plot_rows = []
    outcome_labels = {
        "both_called": "Both called web search",
        "baseline_only": "Only baseline called web search",
        "neither_called": "Neither called web search",
        "prompt_only": "Only system prompt called web search",
    }
    for prompt_version in comparison_versions:
        prompt_called = merged_df[prompt_version].astype(bool)
        prompt_not_called = ~prompt_called
        prompt_label = _prompt_label(prompt_version)
        plot_rows.extend(
            [
                {
                    "system_prompt": prompt_label,
                    "outcome": outcome_labels["both_called"],
                    "count": int((baseline_called & prompt_called).sum()),
                },
                {
                    "system_prompt": prompt_label,
                    "outcome": outcome_labels["baseline_only"],
                    "count": int((baseline_called & prompt_not_called).sum()),
                },
                {
                    "system_prompt": prompt_label,
                    "outcome": outcome_labels["neither_called"],
                    "count": int((baseline_not_called & prompt_not_called).sum()),
                },
                {
                    "system_prompt": prompt_label,
                    "outcome": outcome_labels["prompt_only"],
                    "count": int((baseline_not_called & prompt_called).sum()),
                },
            ]
        )

    plot_df = pd.DataFrame(plot_rows)
    y_labels = [_prompt_label(prompt_version) for prompt_version in comparison_versions]
    outcome_specs = [
        (outcome_labels["both_called"], "#2ca02c"),
        (outcome_labels["baseline_only"], "#d62728"),
        (outcome_labels["neither_called"], "#1f77b4"),
        (outcome_labels["prompt_only"], "#ff7f0e"),
    ]

    fig = go.Figure()
    for outcome, color in outcome_specs:
        subset = plot_df[plot_df["outcome"] == outcome].set_index("system_prompt")
        values = [int(subset.loc[label, "count"]) for label in y_labels]
        fig.add_trace(
            go.Bar(
                y=y_labels,
    ...
    fig.update_layout(
        barmode="stack",
        xaxis_title="Samples",
        yaxis_title="System Prompt",
        legend_title="Web-Call Decision",
        margin=dict(l=5, r=20, t=100, b=80),
    )
    fig = with_paper_style(fig, config=styler(20, 18))
    fig.update_layout(
    ...
    try:
        ...
        first_error_line = str(e).strip().splitlines()[0] if str(e).strip() else e
        print(f"Could not write PDF; HTML was saved. {first_error_line}")
    return summary_df
...
if __name__ == "__main__":
    ...
    # replay_model_call_outcome_venn_diagram()
